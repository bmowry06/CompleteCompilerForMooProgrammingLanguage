...
class FormalsListNode extends ASTnode {
	...
	public void codeGen(PrintWriter p) {
		...
		Codegen.p = p;
		Codegen.generate("addu", Codegen.FP, Codegen.SP, Integer.toString(offSet));
	}
...
class FnBodyNode extends ASTnode {
    ...
    public void nameAnalysis(SymTable symTab, int fOffSet) {
		...
		this.localSpace = fOffSet - offSet - 8;
    }    
	...
	public void codeGen(PrintWriter p, String exit) {
		Codegen.p = p;
		if (this.localSpace > 0) {
			Codegen.generate("subu", Codegen.SP, Codegen.SP, this.localSpace);
		}
		
		myStmtList.codeGen(p, exit);
	}
	...
	public void codeGen(PrintWriter p, String exit) {
        for (StmtNode node : myStmts) {
			if (node instanceof ReturnStmtNode) {
				((ReturnStmtNode)node).codeGen(p, exit);
			} else {
				node.codeGen(p);
			}
        }
...
class VarDeclNode extends DeclNode {
	...
	public int setOffSet(int begin) {
		SemSym s1 = myId.sym();
		int size = 0;
		if (s1 instanceof StructSym) {
			SemSym tempSym = ((StructSym)s1).getStructType().sym();
			size = tempSym.size;
			s1.offSet = begin;
...
class FnDeclNode extends DeclNode {
	...
	public void codeGen(PrintWriter p) { 
		if (myId.name().equals("main")) {
			p.println("main:");
			p.println("__start:");
			p.println("\t\t#Begin Function Prologue");
		} else {
			p.print("\t.text\n" + "_" + myId.name() + ":");
			p.println("\t\t#Begin Function Prologue");
		}
		...
		p.println("\t\t#Begin Function Body");
		
		String exit = Codegen.nextLabel();
		myBody.codeGen(p, exit);
		
		p.println("\t\t#Begin Function Epilogue");
		Codegen.genLabel(exit);
		int rOffSet = ((FnSym)myId.sym()).formalSpace * (-1);
		int fpOffSet = rOffSet - 4;
		
		Codegen.generateIndexed("lw", "$ra", "$fp", rOffSet);
		Codegen.generateWithComment("move", "", "$t0", "$fp");
		Codegen.generateIndexed("lw","$fp", "$fp", fpOffSet, "");
		Codegen.generateWithComment("move", "", "$sp", "$t0");
		
		if(myId.name().equals("main")) {
		   	Codegen.generateWithComment("li", "","$v0", "10");
    ...
    public SemSym nameAnalysis(SymTable symTab) {
        ...
        if (!badDecl) {
            try {   // add entry to symbol table
				...
				sym.size = 0 - myDeclList.setOffSet(0);
                symTab.addDecl(name, sym);
                myId.link(sym);
            } catch (DuplicateSymException ex) {
                System.err.println("Unexpected DuplicateSymException " +
...
abstract class StmtNode extends ASTnode {
	...
	public int setOffSet(int begin) {return begin;}
}
    ...
    public void unparse(PrintWriter p, int indent) {
 	...
 	public void codeGen(PrintWriter p){
   		...
   		if(myExp instanceof IdNode){
   		    ...
   		    Codegen.generateIndexed("sw", "$t0", "$fp", ((DotAccessExpNode)myExp).dot());
   		}
...
class WriteStmtNode extends StmtNode {
	...
	public void codeGen(PrintWriter p){
		...
		if (writeType instanceof IntType) {
		    ...
		    Codegen.generate("li", "$v0", "4", "\t\t#System call for printing string");
		} else if (writeType instanceof BoolType) {
		    Codegen.generate("li", "$v0", "1");
		} else {
		    ErrMsg.fatal(0,0,"error in writestmt while generating assembly" + writeType);
		    System.exit(-1);
		}
		Codegen.generate("syscall");
...
class IfStmtNode extends StmtNode {
    ...
    public int setOffSet(int begin) {
		begin = myDeclList.setOffSet(begin);
		begin = myStmtList.setOffSet(begin);
		return begin;
    }
...
class IfElseStmtNode extends StmtNode {
    ...
    public int setOffSet(int begin) {
		begin = myThenDeclList.setOffSet(begin);
		begin = myThenStmtList.setOffSet(begin);
		begin = myElseDeclList.setOffSet(begin);
		begin = myElseStmtList.setOffSet(begin);
		return begin;
	}
	
    public void codeGen(PrintWriter p){
   		String trueL = Codegen.nextLabel();
   		String doneL = Codegen.nextLabel();
   		myExp.codeGen(p);
    	
   		Codegen.p = p;
   		Codegen.genPop("$t0");
   		Codegen.generate("beq", "$t0", "0", trueL);
   		myThenStmtList.codeGen(p, trueL);
   		Codegen.generate("b", doneL);
   		Codegen.genLabel(trueL);
   		myElseStmtList.codeGen(p, trueL);
   		Codegen.genLabel(doneL);
    }
...
class DotAccessExpNode extends ExpNode {
	...
	public int dot(){
		if (myLoc instanceof IdNode) {
		    ...
		    return DA.dot() + ((IdNode)myId).sym().structOffSet;
		} else{ 
		    return -1;
		}
	 }
...
class AssignNode extends ExpNode {
    ...
    public void codeGen(PrintWriter p) {
		...
		if (myLhs instanceof IdNode) {
		    ...
		    Codegen.generateIndexed("sw", "$t0", "$fp", ((DotAccessExpNode)myLhs).dot());
...
class AndNode extends LogicalExpNode {
    ...
    public void codeGen(PrintWriter p){
		...
		myExp2.codeGen(p);
		Codegen.generate("b", doneL);	
		Codegen.genLabel(trueL, "&& left is false, push false");
		Codegen.genPush("$t0");
		Codegen.genLabel(doneL, "&& is done");
	}
}
...
class OrNode extends LogicalExpNode {
    ...
    public void codeGen(PrintWriter p){
		...
		String trueL = Codegen.nextLabel();
		String doneL = Codegen.nextLabel();
		Codegen.generate("beq", "$t0", "1", trueL);	
		myExp2.codeGen(p);
		Codegen.generate("b", doneL);
		Codegen.genLabel(trueL, "|| left is true, push true");
		Codegen.genPush("$t0");
		Codegen.genLabel(doneL, "|| is done");
	}	
}
...
class EqualsNode extends EqualityExpNode {
    ...
    public void codeGen(PrintWriter p){
   		...
   		if (myExp1 instanceof StringLitNode && myExp2 instanceof StringLitNode) {
   		    String se1 = ((StringLitNode)myExp1).stringValue();
   		    String se2 = ((StringLitNode)myExp2).stringValue();
   		    if (se1.equals(se2)) {
   				Codegen.generate("li","$t0",0);
   				Codegen.genPush("$t0");
